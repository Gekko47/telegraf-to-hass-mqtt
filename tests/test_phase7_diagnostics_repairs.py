...
from custom_components.telegraf_mqtt import repairs
...
def test_parser_stats_count_unknown_measurement_fallback() -> None:
    ...
    parser.parse(
        b'{"name": "custom_plugin", "tags": {"host": "h1"},'
        b' "fields": {"x": 1.0}, "timestamp": 1}'
    )
...
def test_parser_last_message_captures_relevant_metadata() -> None:
    ...
    parser = TelegrafParser(stats=stats)
    parser.parse(
        b'{"name": "cpu", "tags": {"host": "h1"},'
        b' "fields": {"usage_idle": 88.4}, "timestamp": 1}',
...
class _FakeIr:
    ...
    IssueSeverity: type = field(
        default_factory=lambda: types.SimpleNamespace(WARNING="warning")
    )
...
def test_overlap_detection_raises_repair_issue(monkeypatch) -> None:
    ...
    assert any(
        c.issue_id == "overlap_topic_patterns_A_B" for c in fake_ir.created
    )
...
def test_overlap_auto_resolves_when_fixed(monkeypatch) -> None:
    ...
    assert fake_ir.created == []
    assert any(
        d.issue_id == "overlap_topic_patterns_A_B" for d in fake_ir.deleted
    )
...
def test_invalid_persisted_option_no_keys_clears_issue(monkeypatch) -> None:
    ...
    assert fake_ir.created == []
    assert any(
        d.issue_id == "invalid_persisted_option_E" for d in fake_ir.deleted
    )
...
def test_diagnostics_options_validity_marks_invalid_topic_and_overrides() -> None:
    ...
    v = _options_validity(
        {"topic_pattern": "abc\x00def", "field_overrides": {"f": "not-a-dict"}}
    )
    assert v["topic_pattern"] is False
    assert v["field_overrides"] is False

    v2 = _options_validity(
        {"topic_pattern": "ok/topic", "field_overrides": {"f": {"unit": "%"}}}
    )
...
def test_options_from_entry_with_repair_no_invalid_clears_issue(monkeypatch) -> None:
    ...
    integration_module = importlib.import_module(
        "custom_components.telegraf_mqtt.__init__"
    )
    ...
    class _E:
        entry_id: str = 'E'
        options: dict = field(
            default_factory=lambda: {'expire_after': 60, 'cleanup_delay': 10}
        )

    options, invalid = integration_module._options_from_entry_with_repair(
        object(), _E()
    )
    assert invalid == []
